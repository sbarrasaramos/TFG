...
We want to quickly recall the aim of our experiments and the protocol followed in each of them:

\begin{itemize}
	\item Regarding the aim, we intend to study the influence of \textit{Chlamydomonas reinhardtii} on the diffusion of micrometric tracers in a 2-dimensional environment. 
	\item Concerning the protocol, we first prepare an aliquot of the algae in their medium (higher levels of concentration are achieved by centrifugation). Then, we add the tracers and, if required, the surfactant. Some wet cotton is to be put in the box if we want to prevent evaporation. We reduce the size of the square to its minumum by turning the screw and, with the help of a pipette, we put a droplet of the desired volume between the threads, touching them. With the help of the device, we pull the threads $6 \; \textrm{mm}$ away, so as to have a thin square film, and we cover the box. Finally we turn the box upside down and we put it under the microscope, where we will wait for it to reach the expected thickness. Then we acquire images for approximately 1 minute. One of the resulting images can be seen on Fig. \ref{frame_e3}.
\end{itemize}
...
As it has been explained in Section \ref{brownian_diff}, diffusion can be quantified by measuring the MSD of tracers as a function of the elapsed time. This is why, in our images, we will look for the displacements, $\Delta x$ and $\Delta y$, of the tracers along the axes of the selected reference frame (usually the laboratory reference frame) for different elapsed times, $\Delta t$.

Even if, among our experiments, most were devoted to assess the diffusion of tracers in the presence of \textit{Chlamydomonas reinhardtii}, a non-negligible amount of them had other purposes such as evaluating the effects of evaporation, surfactants, tilt of the device,... This already reduces the number of available data, but much more importantly, we have not been able of exploiting most of the images satisfactorily.

The presence of this effects mostly affects our analysis through the apparition of \textit{background flows}, which we explain in the following section. 

\section{Background flow}

Fig. \ref{backg_flow} shows a projection of the darkest points of a series of frames (over $5.5$ seconds). This allows us to see the particle and algae paths. The particle paths in Fig. \ref{backg_flow} show a \textit{mean} behavior that is neither Brownian nor related to the algae kicks.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.65\textwidth]{archivos/backg_flow.png}
	\caption{The common trend of the streamlines (over 5.5 seconds) in the left part of the image suggests the presence of a background flow. It reflects a velocity magnitude of the order of $6 \mu m / s$ (thick paths correspond to algae, and thin paths, to particles).}
	\label{backg_flow}
\end{figure}

As we have already mentioned in Section \ref{flowing_films}, e call \textit{background flow} to these currents, whose origin is unrelated to the presence of the algae.

If the background flow were uniform, we could think of simply substracting its global mean numerically, but it is not the case as one can see from the figure above. 
...
\fbox{
	\begin{minipage}{15.4 cm}
		...
		\textbf{Initialisation:} reading of the trajectories data (algae and tracers)
		
		\vspace{0.2 cm}
		
		\textbf{Removal of the algae's Brownian motion and noise:}
		application of a filter to remove the Brownian motion from the algae trajectories
		...
		\quad \textbf{Preparing parallelization:} partitioning the data
...
\begin{figure}[H]
	\centering
	\includegraphics[height=0.49\textwidth]{archivos/frame_01_e3.png}
	\includegraphics[height=0.491\textwidth]{archivos/frame_01_e3_bp.png}
...
\begin{itemize}
	...
	\item equalizing\footnote{Histogram equalization of a set of images is a method that allows to adjust the intensities of each image to make them similar between them.} the set of images\footnote{If intensity varies from image to image the intensity threshold used by the tracking algorithm may not be effective},
...
To summarize last section: we set thicker films and wait for them to thin thanks to evaporation. As a consequence, further problems arise, that we explain hereafter.
